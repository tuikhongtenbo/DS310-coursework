...
def main():
    parser = argparse.ArgumentParser(description="Qwen inference with vLLM (Vietnamese QA)")
    parser.add_argument(
        "--input_file", 
        type=str, 
    ...
    parser.add_argument("--output_file", type=str, default="./predictions/predictions_qwen.json")
    parser.add_argument(
        "--model_name", type=str, default="Qwen/Qwen2.5-7B-Instruct-AWQ", help="HF model name for Qwen"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    # Setup logger
    logger = setup_logger("qwen_infer", log_dir=args.log_dir)
